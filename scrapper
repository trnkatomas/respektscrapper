...
class Issue():
    ...
    def __init__( self ):
        ...
        self.urls_clanku = [art.get_attribute( "href" ) for art in allArts]

    def check_issue( self ):
        with open( args.history_filename, "r", encoding = "utf8" ) as histfile:
            first_line_raw = histfile.readline()[:-1]  # nechci \n
        first_line = first_line_raw.split( '/' )
        first_issue_raw = self.get_cisla()[0]
        first_issue = first_issue_raw.text.split( '/' )
        href = first_issue_raw.get_attribute( "href" )


        if first_line == first_issue:
            return( False, None )
        elif int( first_line[1] ) < int( first_issue[1] ):  # roky
            return( True, href )
        elif int( first_line[0] ) < int( first_issue[0] ):  # cislo
            return( True, href )
        else:
    ...
    def make_epub( self ):
        ...
        ftoc = []
        for art in self.content:
            c = epub.EpubHtml( title = art["title"], file_name = "{0}.xhtml".format( "_".join( art["title"].split() ) ) )
        ...
        for img in os.listdir( "images" ):
            ...
            im = epub.EpubItem( file_name = pathe, media_type = "image/jpg" )
